                ...
                <div class="col-md-12">
                    <div class="invoice p-3 mb-3" id="withoutlogo1">
                        <div class="row no-print">
                            <div class="col-12">
                                <button type="button" class="btn btn-default float-right" onclick="printWithoutlogo()"><i
                                        class="fas fa-print"></i> Print Invoice</button>
                                <button type="button" class="btn btn-default float-right" onclick="printWithlogo()"><i
                                        class="fas fa-print"></i> Print Invoice With Logo's</button>
                            </div>
                        </div>
                        ...
                        <br>
                        <h5 class="text-center text-weight-bold pb-1"
                            style="margin-top:10px;margin-bottom:10px;text-transform: uppercase;">Tax @lang('models/invoices.singular')
                        ...
                        <table class="table  mb-4 expandable-table" border="1">
                            <thead>
                                <tr class="k-bg-gy">
                                    @if ($invoice->type == 'rental')
                                        <th>@lang('models/products.fields.start_date') @lang('models/products.fields.time')</th>
                                        {{-- <th>@lang('models/products.fields.time')</th> --}}
                                        <th>@lang('models/products.fields.end_date') @lang('models/products.fields.time')</th>
                                        {{-- <th>@lang('models/products.fields.time')</th> --}}
                                    @endif
                                    ...
                                    <th>@lang('models/products.fields.vat')</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($products as $product)
                                    @php
                                        $total_vat += $product->amount * 0.05;
                                        $total_qty += $product->qty;
                                        $total_amount += $product->amount + $product->amount * 0.05;
                                    @endphp
                                    <tr class="expandable-header">
                                        @if ($invoice->type == 'rental')
                                            <td>{{ $product->start_date->format('Y-m-d') . ' ' . $product->start_date->format('H:i') }}
                                            </td>
                                            {{-- <td>{{ $product->start_date->format('H:i') }}</td> --}}
                                            <td>{{ $product->end_date->format('Y-m-d') . ' ' . $product->end_date->format('H:i') }}
                                            </td>
                                        ...
                                        @endif
                                        <td>{{ $product->price }}</td>
                                        <td>{{ $product->amount * 0.05 }}</td>
                                        <td>{{ $product->qty }}</td>
                                        <td>{{ $product->amount + $product->amount * 0.05 }}</td>
                                    </tr>
                                @endforeach
                        ...
                        <table class="table mb-4" border="1"
                            style="width:40% !important;    margin: auto;right: 0;margin-right: 0;">
                            <thead>
                                <tr class="k-bg-gy">
                                    {{-- <th>@lang('models/invoices.fields.discount')</th> --}}
                                    <th>
                                        {{-- @lang('models/invoices.fields.gross') --}}
                                        Taxable Amount
                                    </th>
                                    <th>
                                        {{-- @lang('models/invoices.fields.vat') --}}
                                        Tax Amount
                                    </th>
                                    <th>
